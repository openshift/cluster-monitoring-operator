...
func (rc *RuleController) sync(ctx context.Context, key string) error {
	...
	// TODO: Remove in 4.23.
	// Temporary: Clean up any stale PrometheusRule left over from the MD5-based naming
	// scheme that was in use before the hash function was changed to SHA-224
	// in https://github.com/openshift/cluster-monitoring-operator/pull/2086
	legacyName := LegacyPromRuleName(rule.Namespace, rule.Name, rule.UID)
	if legacyName != promRule.Name {
		if err := rc.client.DeletePrometheusRuleByNamespaceAndName(ctx, rc.client.Namespace(), legacyName); err != nil {
			return fmt.Errorf("error cleaning up legacy PrometheusRule %q: %w", legacyName, err)
		}
	}
...
// NOTE: The output must be stable, a change will result in duplicate PrometheusRules
...
// LegacyPromRuleName returns the PrometheusRule name that would have been
// generated by the old MD5-based naming scheme. Used only to clean up stale
// PrometheusRules left behind after the hash function was changed to SHA-224
// in https://github.com/openshift/cluster-monitoring-operator/pull/2086.
func LegacyPromRuleName(namespace, name string, uid types.UID) string {
	data := fmt.Sprintf("%s-%s-%s", namespace, name, uid)
