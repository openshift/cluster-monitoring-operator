...
|ethtool|link:nodeexportercollectorethtoolconfig.adoc[NodeExporterCollectorEthtoolConfig]|Defines the configuration of the `ethtool` collector, which collects ethernet device statistics. Disabled by default.
...
|interrupts|link:nodeexportercollectorinterruptsconfig.adoc[NodeExporterCollectorInterruptsConfig]|Defines the configuration of the `interrupts` collector, which exposes interrupt counts from `/proc/interrupts`. Disabled by default.
...
|dmMultipath|link:nodeexportercollectordmmultipathconfig.adoc[NodeExporterCollectorDmMultipathConfig]|Defines the configuration of the `dmmultipath` collector, which exposes DM-multipath device and path metrics from `/sys/block/dm-*`. Enabled by default.

|nvmeSubsystem|link:nodeexportercollectornvmesubsystemconfig.adoc[NodeExporterCollectorNvmeSubsystemConfig]|Defines the configuration of the `nvmesubsystem` collector, which exposes NVMe subsystem metrics from `/sys/class/nvme-subsystem/`. Enabled by default.
