...
data:
  metrics.yaml: |-
    ...
    # ocs:node_num_cpu:sum gives the total number of CPU present on the 
    # nodes which are being used for OCS.
    - '{__name__="ocs:node_num_cpu:sum"}'
    # ocs:container_cpu_usage:sum gives the total CPU usage by OCS components.
    - '{__name__="ocs:container_cpu_usage:sum"}'
