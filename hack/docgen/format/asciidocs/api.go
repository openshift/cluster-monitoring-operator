...
[id="clustermonitoring-custom-resource-techpreview"]
...
In addition to config maps, platform monitoring components can be configured through a cluster-scoped ` + "`ClusterMonitoring`" + ` custom resource named ` + "`cluster`" + ` in the ` + "`config.openshift.io/v1alpha1`" + ` API group. The CRD is defined in the https://github.com/openshift/api/blob/master/config/v1alpha1/types_cluster_monitoring.go[openshift/api] repository.

During Phase 1 (pre-GA), the config map takes precedence: for each top-level component, if the config map sets a value, the corresponding CRD field is ignored. CRD values are only applied when the config map does not configure that component.

Detailed user workload monitoring configuration (` + "`user-workload-monitoring-config`" + `) is not yet configurable through a CRD and remains config map only, though enablement (` + "`enableUserWorkload`" + `) can be set via the CRD's ` + "`userDefined.mode`" + ` field.
`
