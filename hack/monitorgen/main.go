...
func main() {
	...
	if val, ok := m.ObjectMeta.Annotations[collectionProfileLabel]; ok && val != "full" {
		return
	}
...
// generateCollectionProfileMinimal takes as input a ServiceMonitor (SM) and
// generates the minimal version of that SM
func generateCollectionProfileMinimal(m monitoringv1.ServiceMonitor) {
	...
	if *minimalMetricsFilePath != "" {
		metricsToKeep, err = readMetricsFromFile(*minimalMetricsFilePath)
		if err != nil {
	...
	mMinimal.Labels[collectionProfileLabel] = "minimal"
