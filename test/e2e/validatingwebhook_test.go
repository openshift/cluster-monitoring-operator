...
`, framework.E2eTestLabel)
	...
	// Test UTF-8 characters in AlertmanagerConfig
	utf8AmConf = fmt.Sprintf(`---
apiVersion: monitoring.coreos.com/v1beta1
kind: AlertmanagerConfig
metadata:
  name: cluster.alerts-utf8
  labels:
    %s
spec:
  route:
    ...
    groupInterval: 5m
    repeatInterval: 12h
    receiver: 'cluster.webhook'
  receivers:
  - name: 'cluster.webhook'
    webhookConfigs:
    - url: 'https://example.com'
      title: 'Cluster Alert'
      description: 'Cluster monitoring alert with UTF-8'
`, framework.E2eTestLabel)
)
...
func TestPrometheusRuleValidatingWebhookUTF8Names(t *testing.T) {
	...
	// Test AlertmanagerConfig with UTF-8 names.
	utf8AmConfig := monitoringv1beta1.AlertmanagerConfig{}
	err = yaml.Unmarshal([]byte(utf8AmConf), &utf8AmConfig)
	if err != nil {
		t.Fatal("unable to unmarshal UTF-8 alertmanager config", err)
	}

	_, err = f.MonitoringBetaClient.AlertmanagerConfigs(f.Ns).Create(ctx, &utf8AmConfig, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("UTF-8 alertmanager config was rejected by validating webhook: %v", err)
	}

	f.MonitoringClient.PrometheusRules(f.Ns).Delete(ctx, utf8PromRule.Name, metav1.DeleteOptions{})
	f.MonitoringBetaClient.AlertmanagerConfigs(f.Ns).Delete(ctx, utf8AmConfig.Name, metav1.DeleteOptions{})
}
