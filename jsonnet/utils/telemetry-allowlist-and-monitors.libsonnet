...
// Each entry in the whitelist will continue to adhere to the existing pattern, i.e.,
// #
  ...
  local matches = std.join('\n', std.flatMap(
    function(entry)
      ...
      (if std.length(std.objectFields(label_values)) == 0 then
         ...
         std.flatMap(
           function(key)
             std.map(function(v) tab + '# - %s: %s' % [key, v], label_values[key]),
           std.objectFields(label_values)
         ))
      +  // source monitors and metrics information
      (
        local monitor_keys = std.objectFields(monitor_metrics);
        if std.length(monitor_keys) == 1 && monitor_keys[0] == '' && std.length(monitor_metrics['']) == 0 then  // { '': [] }
          [
            tab + '#',
            tab + '# This rule is a metric in itself.',
          ]
        else
          [
            tab + '#',
