...
local unknownMonitorIgnorelist = {
  ...
  '{__name__="cluster:kubernetes_nmstate_features_applied:sum",name=~"sriov|ovs-dpdk|ovn-mapping|ovs-bond|ovs-patch|lldp|mptcp|static-route-rule|iface-name-refered-by-sriov-vf-id|mac-based-identifier"}': true,
  '{__name__="cluster:kubernetes_nmstate_network_interfaces:sum",type=~"ethernet|linux-bridge|bond|vlan|ovs-bridge|ovs-interface|ovs-port|mac-vlan|vxlan|infiniband"}': true,
  '{__name__="cluster:kubernetes_nmstate_routes:sum"}': true,
  '{__name__="cluster:ovnkube_clustermanager_user_defined_networks:max",role=~"Primary|Secondary",topology=~"Layer2|Layer3"}': true,
  '{__name__="cluster:ovnkube_clustermanager_cluster_user_defined_networks:max",role=~"Primary|Secondary",topology=~"Layer2|Layer3"}': true,
  '{__name__="cluster:k8s_mcp_tool_calls:sum"}': true,
  '{__name__="cluster:k8s_mcp_tool_errors:sum"}': true,
  '{__name__="cluster:k8s_mcp_http_requests:sum"}': true,
};
...
local entries = [
  ...
  makeEntry({
    metadata: {
      owners: ['@openshift/openshift-team-monitoring'],
      description: 'collector:node_scrape_collector_success:avg reports the average success of optional node_exporter collectors running in the cluster.',
      label_values: {
        collector: ['ethtool', 'cpufreq', 'tcpstat', 'netdev', 'netclass', 'buddyinfo', 'mountstats', 'ksmd', 'processes', 'systemd', 'softirqs'],
      },
      consumers: [],
    },
    rule: '{__name__="collector:node_scrape_collector_success:avg",collector=~"ethtool|cpufreq|tcpstat|netdev|netclass|buddyinfo|mountstats|ksmd|processes|systemd|softirqs"}',
    monitor_metrics: {
      [nodeExporterMonitorKey]: ['node_scrape_collector_success'],
    },
  }),
  makeEntry({
    metadata: {
      owners: ['@openshift/networking'],
      description: 'cluster:kubernetes_nmstate_features_applied:sum gives the total count of nmstate features applied across all nodes in the cluster, broken down by feature name.',
      label_values: {
        name: ['sriov', 'ovs-dpdk', 'ovn-mapping', 'ovs-bond', 'ovs-patch', 'lldp', 'mptcp', 'static-route-rule', 'iface-name-refered-by-sriov-vf-id', 'mac-based-identifier'],
      },
      consumers: [],
    },
    rule: '{__name__="cluster:kubernetes_nmstate_features_applied:sum",name=~"sriov|ovs-dpdk|ovn-mapping|ovs-bond|ovs-patch|lldp|mptcp|static-route-rule|iface-name-refered-by-sriov-vf-id|mac-based-identifier"}',
    monitor_metrics: {
  ...
  makeEntry({
    metadata: {
      owners: ['@openshift/networking'],
      description: 'cluster:kubernetes_nmstate_network_interfaces:sum gives the total count of network interfaces across all nodes in the cluster, broken down by interface type.',
      label_values: {
        type: ['ethernet', 'linux-bridge', 'bond', 'vlan', 'ovs-bridge', 'ovs-interface', 'ovs-port', 'mac-vlan', 'vxlan', 'infiniband'],
      },
      consumers: [],
    },
    rule: '{__name__="cluster:kubernetes_nmstate_network_interfaces:sum",type=~"ethernet|linux-bridge|bond|vlan|ovs-bridge|ovs-interface|ovs-port|mac-vlan|vxlan|infiniband"}',
    monitor_metrics: {
      '': ['kubernetes_nmstate_network_interfaces'],
    },
  }),
  makeEntry({
    metadata: {
      owners: ['@openshift/networking'],
      description: 'cluster:kubernetes_nmstate_routes:sum gives the total count of network routes across all nodes in the cluster, broken down by IP stack and route type.',
      label_values: {},
      consumers: [],
    },
    rule: '{__name__="cluster:kubernetes_nmstate_routes:sum"}',
    monitor_metrics: {
      '': ['kubernetes_nmstate_routes'],
    },
  }),
  makeEntry({
    metadata: {
      owners: ['@openshift/networking'],
      description: 'cluster:ovnkube_clustermanager_user_defined_networks:max gives the count of UserDefinedNetworks objects in the cluster, broken down by role and topology.',
      label_values: {
        role: ['Primary', 'Secondary'],
        topology: ['Layer2', 'Layer3'],
      },
      consumers: [],
    },
    rule: '{__name__="cluster:ovnkube_clustermanager_user_defined_networks:max",role=~"Primary|Secondary",topology=~"Layer2|Layer3"}',
    monitor_metrics: {
  ...
  makeEntry({
    metadata: {
      owners: ['@openshift/networking'],
      description: 'cluster:ovnkube_clustermanager_cluster_user_defined_networks:max gives the count of ClusterUserDefinedNetworks objects in the cluster, broken down by role and topology.',
      label_values: {
        role: ['Primary', 'Secondary'],
        topology: ['Layer2', 'Layer3'],
      },
    ...
    rule: '{__name__="cluster:ovnkube_clustermanager_cluster_user_defined_networks:max",role=~"Primary|Secondary",topology=~"Layer2|Layer3"}',
    monitor_metrics: {
  ...
  makeEntry({
    metadata: {
      ...
      description: 'cluster:k8s_mcp_tool_calls:sum tracks the total number of MCP tool calls.',
      label_values: {},
      consumers: [],
    },
    rule: '{__name__="cluster:k8s_mcp_tool_calls:sum"}',
    monitor_metrics: {
      '': ['k8s_mcp_tool_calls'],
    },
  }),
  makeEntry({
    metadata: {
      owners: ['https://github.com/containers/kubernetes-mcp-server'],
      description: 'cluster:k8s_mcp_tool_errors:sum tracks the total number of MCP tool errors.',
      label_values: {},
      consumers: [],
    },
    rule: '{__name__="cluster:k8s_mcp_tool_errors:sum"}',
    monitor_metrics: {
      '': ['k8s_mcp_tool_errors'],
    },
  }),
  makeEntry({
    metadata: {
      owners: ['https://github.com/containers/kubernetes-mcp-server'],
      description: 'cluster:k8s_mcp_http_requests:sum tracks the total HTTP requests.',
      label_values: {},
      consumers: [],
    },
    rule: '{__name__="cluster:k8s_mcp_http_requests:sum"}',
    monitor_metrics: {
      '': ['k8s_mcp_http_requests'],
    },
  }),
];
