...
local configDir = '/var/node_exporter/config';
local configVolumeName = 'node-exporter-config';
local acceleratorsConfigFileName = 'accelerators-collector-config.yaml';
...
function(params)
  ...
  nodeExporter(cfg) {
    ...
    acceleratorsCollectorConfigmap: {
      ...
      data: {
        'accelerators-collector-config.yaml': std.manifestYamlDoc(acceleratorsConfigData),
      },
    },
  }
