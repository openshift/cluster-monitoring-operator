      ...
      if std.member(['node', 'instance'], t.name) then
        [
          template.new(
            ...
            query='label_values(kube_node_role{%(clusterLabel)s="$cluster"} * on(node) group_left kube_node_labels{%(clusterLabel)s="$cluster", label_topology_kubernetes_io_zone=~".+"}, role)' % glib._config,
            allValues='.+',
            current='all',
            hide='',
            refresh=2,
          ...
          template.new(
            name=t.name,
            datasource='$datasource',
            query='label_values(kube_node_info{%(clusterLabel)s="$cluster"} * on (node) group_right() (kube_node_role{%(clusterLabel)s="$cluster", role=~"$role"} * on(node) group_left kube_node_labels{%(clusterLabel)s="$cluster", label_topology_kubernetes_io_zone=~".+"}), node)' % glib._config,
            current='',
            hide='',
            refresh=2,
            includeAll=false,
